...
rotateOptions.forEach(option => {
    ...
    if(option.id === 'left') {
        rotate += 90; // if clicked btn is left rotate, decremment rotate value by -90 
    } else if(option.id === 'right') {
        rotate -= 90; // if clicked btn is right rotate, increment rotate value by +90
    } else if(option.id === 'horizontal') {
        // if flipVertical value is 1, set this valur to -1  else set 1
        // flipHorizontal = flipHorizontal === 1 ? -1 : 1;
        flipVertical = flipVertical === 1 ? -1 : 1;
    } else {
         // if flipHorizontal value is 1, set this valur to -1  else set 1
        flipHorizontal = flipHorizontal === 1 ? -1 : 1;
    }
    applyFilters();
    });
...
const saveImage = () => {
    ...
    const ctx = canvas.getContext('2d'); // canvas.getConteext return a drawing context on the canvas
    canvas.width = previewImg.naturalWidth; // setting canvas width to  actual image width
    canvas.height = previewImg.naturalHeight; // setting canvas height to actual height
